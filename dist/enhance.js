...
const anti_slop_1 = require("./anti-slop");
/**
 * Analyze a prompt and return enhancement suggestions
 */
function enhance(prompt, options) {
    const slop = (0, anti_slop_1.detectSlop)(prompt);
    const score = (0, anti_slop_1.slopScore)(prompt);
    const palettes = (0, palettes_1.detectPalette)(prompt);
    const suggestions = [];
    ...
    // Quality level
    let qualityLevel = 'generic';
    if (hasSpecificDetail || hasProperNouns)
        qualityLevel = 'clear';
    if (qualityLevel === 'clear' && sensoryWords.test(prompt))
        qualityLevel = 'vivid';
    if (qualityLevel === 'vivid' && hasNegation && slop.length === 0)
        qualityLevel = 'resonant';
    return {
        original: prompt,
        slopMatches: slop,
        slopScore: score,
